...
namespace cookie
{
    ...
    public:
        ...
        template<class T>
        World* AddSystem(T system, bool callStart = false)
        {
        ...
        Entity EnqueueEntitySpawn(Components... components)
        {
            auto f = std::bind(&World::spawnEntity<Components...>, this, components...);
            commands.push_back(f);
            u32 index = despawnedEntities.size() > 0 ?
                despawnedEntities.at(enqueuedIndices) 
                : EntityCount + enqueuedIndices;
            u32 gen = despawnedEntities.size() > 0 ? currentEntities.at(index).gen + 1 : 0;
            enqueuedIndices++;
    ...
    private:
        ...
        Query<QueryTypes...> queryEntitiesWithPointers(std::vector<std::optional<QueryTypes>>*... vectorPointers) const
        {
            ...
            result.reserve(EntityCount / 2);
            entities.reserve(EntityCount / 2);
